...
Point Display::GetPosition() const {
  if (!pimpl_->h_monitor_) return {0.0, 0.0};
  MONITORINFOEX monitorInfo = GetMonitorInfo(pimpl_->h_monitor_);
...
Rectangle Display::GetWorkArea() const {
  if (!pimpl_->h_monitor_) return {0.0, 0.0, 0.0, 0.0};
  MONITORINFOEX monitorInfo = GetMonitorInfo(pimpl_->h_monitor_);
...
double Display::GetScaleFactor() const {
  if (!pimpl_->h_monitor_) return 1.0;
...
int Display::GetRefreshRate() const {
  return 60; // Default refresh rate, would need additional Windows APIs to get actual value
}
...
std::string Display::GetManufacturer() const {
  return "Unknown"; // Would need additional Windows APIs to get manufacturer info
}
...
std::string Display::GetSerialNumber() const {
  return ""; // Would need additional Windows APIs to get serial number
}
