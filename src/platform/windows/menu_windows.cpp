...
std::pair<UINT, UINT> ConvertAccelerator(const KeyboardAccelerator& accelerator) {
    ...
    if (!accelerator.key.empty()) {
        if (accelerator.key.length() == 1) {
            ...
            if (keyStr == "F1") key = VK_F1;
            else if (keyStr == "F2") key = VK_F2;
            else if (keyStr == "F3") key = VK_F3;
            else if (keyStr == "F4") key = VK_F4;
            else if (keyStr == "F5") key = VK_F5;
            else if (keyStr == "F6") key = VK_F6;
            else if (keyStr == "F7") key = VK_F7;
            else if (keyStr == "F8") key = VK_F8;
            else if (keyStr == "F9") key = VK_F9;
            else if (keyStr == "F10") key = VK_F10;
            else if (keyStr == "F11") key = VK_F11;
            else if (keyStr == "F12") key = VK_F12;
            else if (keyStr == "Enter" || keyStr == "Return") key = VK_RETURN;
            else if (keyStr == "Tab") key = VK_TAB;
            else if (keyStr == "Space") key = VK_SPACE;
            else if (keyStr == "Escape") key = VK_ESCAPE;
            else if (keyStr == "Delete" || keyStr == "Backspace") key = VK_BACK;
            else if (keyStr == "ArrowUp") key = VK_UP;
            else if (keyStr == "ArrowDown") key = VK_DOWN;
            else if (keyStr == "ArrowLeft") key = VK_LEFT;
            else if (keyStr == "ArrowRight") key = VK_RIGHT;
        }
    }
...
class MenuItem::Impl {
 public:
    ...
    ~Impl() {
        // Windows menu items are automatically cleaned up when the menu is destroyed
    }
};
